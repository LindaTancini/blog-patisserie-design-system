...
const meta: Meta = {
  title: "Foundations/Colors", // Titolo della storia nella sidebar di Storybook
  parameters: {
    ...
    docs: {
      description: {
        story:
          ...
          "Sono presenti tonalità di grigi, rosa zucchero e lilla crema, utili per UI components e sfondi.",
      },
    },
  },
  ...
  render: () => (
    <div>
      ...
      <div className="container">
        {Array.from({ length: 16 }).map((_, i) => (
          <ClickToCopy
            style={{ backgroundColor: `var(--color-pink-${i})` }} // Sfondo colore rosa zucchero
            key={i} // Chiave unica per React
      ...
      <div className="container">
        {Array.from({ length: 16 }).map((_, i) => (
          <ClickToCopy
            style={{ backgroundColor: `var(--color-lilac-${i})` }} // Sfondo colore lilla crema
            key={i} // Chiave unica per React
            value={`--color-lilac-${i}`} // Valore da copiare
          />
        ))}
      </div>
    </div>
